...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField]
    private float horizontalDecceleration; //The speed at which our player decelerates from their current speed.
    ...
    [SerializeField]
    private float jumpTimeToApex; //Time between applying the jump force and reaching the desired jump height.
    ...
    [SerializeField]
    private float coyoteTime; // Grace period after falling off a platform, where you can still jump

    [SerializeField]
    private float jumpCutGravityMult; // Multiplier to increase gravity if the player releases the jump button while still jumping

    [SerializeField]
    private float jumpHangGravityMult; // Reduces gravity while close to the apex (desired max height) of the jump

    [SerializeField]
    private float jumpHangTimeThreshold; // Speeds (close to 0) where the player will experience extra "jump hang".

    [SerializeField]
    private float jumpHangAccelerationMult;
    ...
    [SerializeField]
    private float fallGravityMult; //Multiplier to the player's gravityScale when falling.

    [SerializeField]
    private float maxFallSpeed; //Maximum fall speed (terminal velocity) of the player when falling.

    [SerializeField]
    private float fastFallGravityMult; //Larger multiplier to the player's gravityScale when they press down

    [SerializeField]
    private float maxFastFallSpeed; //Maximum fall speed(terminal velocity) of the player when performing a faster fall.
    ...
    [SerializeField]
    private float respawnFloatTime;

    [SerializeField]
    private float respawnFallGravityMult;

    [SerializeField]
    private float maxRespawnFallspeed;
    ...
    [SerializeField]
    private float LastRespawnedTime;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Space) || Input.GetKeyUp(KeyCode.W) || Input.GetKeyUp(KeyCode.J))
        {
            OnReleaseJumpInput();
        }
        #endregion
        ...
        if (LastRespawnedTime > 0)
        {
            ...
            rb2d.velocity = new Vector2(
                rb2d.velocity.x,
                Mathf.Max(rb2d.velocity.y, -maxRespawnFallspeed)
            );
        }
        ...
        else if (rb2d.velocity.y < 0 && _moveInput.y < 0)
        {
            ...
            rb2d.velocity = new Vector2(
                rb2d.velocity.x,
                Mathf.Max(rb2d.velocity.y, -maxFastFallSpeed)
            );
        }
        else if (_isJumpCut)
        {
            ...
            (!IsGrounded || _isJumpFalling)
            && Mathf.Abs(rb2d.velocity.y) < jumpHangTimeThreshold
        )
        {
    ...
    private void HorizontalMove(float lerpAmount)
    {
        ...
        accelRate =
            (Mathf.Abs(targetSpeed) > 0.01f) ? horizontalAccelAmount : horizontalDeccelAmount;
        #endregion
        ...
        if (
            doConserveMomentum
            && Mathf.Abs(rb2d.velocity.x) > Mathf.Abs(targetSpeed)
            && Mathf.Sign(rb2d.velocity.x) == Mathf.Sign(targetSpeed)
    ...
    private void Jump()
    {
        ...
        if (rb2d.velocity.y < 0)
        {
            currJumpForce -= rb2d.velocity.y;
        }

        rb2d.AddForce(Vector2.up * currJumpForce, ForceMode2D.Impulse);
